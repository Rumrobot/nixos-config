...
export default function Workspaces(props: {
  ...
  const workspaces: Accessor<AstalHyprland.Workspace[]> = createBinding(
    hyprland,
    "workspaces",
  );
  const focused: Accessor<AstalHyprland.Workspace> = createBinding(
    hyprland,
    "focusedWorkspace",
  );
  ...
  return (
    <box
      orientation={
        props.vertical ? Gtk.Orientation.VERTICAL : Gtk.Orientation.HORIZONTAL
      }
      spacing={6}
      hexpand={true}
            ...
            {(value) =>
              focused && (
                <button
                  ...
                  label={`${ws.id}`}
                  class={
                    ws.id === focused.id ? "ws-button focused" : "ws-button"
                  }
                  onClicked={() => ws.focus()}
                />
              )
